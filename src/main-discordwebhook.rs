...
const DISCORD_WEBHOOK_URL: &str = "https://discord.com/api/webhooks/1417554483499503616/YcGBm_EqoY5iHl9Bap9RT0Xeb2xkd9nzZRvnL2rv-WKYMEeKLHc6ZqarWiJoqka-Zl5g";
...
async fn send_to_discord(
    ...
    let mut message = format!("**New Duo Transaction Detected**\n");
    message.push_str(&format!("**Transaction ID:** `{}`\n", transaction.urgid));

    if let Some(summary) = transaction.other.get("summary") {
        message.push_str(&format!("**Summary:** {}\n", summary));
    }

    if let Some(txid) = transaction.other.get("txid") {
        message.push_str(&format!("**TX ID:** `{}`\n", txid));
    }

    if let Some(tx_type) = transaction.other.get("type") {
        message.push_str(&format!("**Type:** {}\n", tx_type));
    }

    if let Some(attributes) = transaction.other.get("attributes") {
        if let Some(attrs_array) = attributes.as_array() {
            message.push_str("\n**Details:**\n");
            for section in attrs_array {
                if let Some(section_array) = section.as_array() {
                    for item in section_array {
                        if let Some(item_array) = item.as_array() {
                            if item_array.len() >= 2 {
                                let key = item_array[0].as_str().unwrap_or("Unknown");
                                let value = match &item_array[1] {
    ...
    let response = client.post(DISCORD_WEBHOOK_URL).json(&payload).send().await?;

    if !response.status().is_success() {
        println!("Failed to send to Discord: Status {}", response.status());
        println!("Response: {}", response.text().await?);
    } else {
        println!("Transaction sent to Discord successfully");
    }
            ...
            if let Some(response) = r.response {
                ...
                if !transactions.is_empty() {
                    for tx in transactions {
                        ...
                        if let Err(e) = send_to_discord(&tx).await {
